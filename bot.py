...
class UserSettings:
    ...
    def get_settings(self, user_id: int) -> dict:
        ...
        if user_id_str not in self.settings:
            ...
            if any(key not in self.settings[user_id_str] for key in default_settings):
                self.save_settings()
    ...
    def update_settings(self, user_id: int, **kwargs):
        ...
        valid_keys = ["stats_public", "history_public", "anonymous", "anon_id"]
...
class GameHistory:
    ...
    def add_game(self, guild_id: int, user_id: int, game_data: dict):
        ...
        game_entry = {
            ...
            "anon_id": settings["anon_id"] if settings["anonymous"] else None
        }
...
class EnhancedLeaderboardView(View):
    ...
    def create_components(self):
        ...
        if self.leaderboard_data:
            ...
            for entry in self.leaderboard_data:
                ...
                settings = self.cog.settings.get_settings(entry["user_id"])
                if user and not settings["anonymous"]:
                    name = user.display_name
                else:
                    name = f"Anonym[{settings['anon_id']}]"
    ...
    async def show_stats(self, interaction: discord.Interaction):
        ...
        for idx, entry in enumerate(sorted_data[:10], 1):
            user = self.cog.bot.get_user(entry["user_id"])
            settings = self.cog.settings.get_settings(entry["user_id"])
            name = f"Anonym[{settings['anon_id']}]" if settings["anonymous"] else user.display_name
            embed.add_field(
                name=f"{idx}. {name}",
    ...
    def create_leaderboard_embed(self):
        embed = discord.Embed(
            title=f"🏆 {get_scope_label(self.scope)} Rangliste (nur öffentliche Spiele)",
            color=discord.Color.gold()
        )
...
class PlayerOptionsView(View):
    def __init__(self, cog, user_id: int, guild_id: Optional[int], scope: str):
        super().__init__(timeout=60)
        self.cog = cog
        self.user_id = user_id
        self.guild_id = guild_id
    ...
    def create_options_embed(self):
        ...
        return discord.Embed(
            title=f"🎮 Optionen für {user.display_name if user else 'Unbekannt'}",
            color=discord.Color.blue()
        )
    
    @ui.button(label="Statistiken", style=discord.ButtonStyle.primary, emoji="📊", custom_id="player_stats")
    ...
    @ui.button(label="Historie", style=discord.ButtonStyle.primary, emoji="📜", custom_id="player_history")
...
class HistoryView(View):
    ...
    def create_history_embed(self) -> discord.Embed:
        ...
        embed = discord.Embed(
            title=f"📜 {get_scope_label(self.scope)} Historie",
            description=f"Seite {self.page + 1}/{total_pages}",
            color=discord.Color.blue()
        )
        ...
        if games and self.page < len(games):
            ...
            embed.add_field(name="Ergebnis", value=f"{status} am {date}", inline=False)
            ...
            embed.add_field(name="Anonym", value="✅ Ja" if game["anonymous"] else "❌ Nein", inline=False)
            if game.get("anon_id"):
                embed.set_footer(text=f"Anonyme ID: {game['anon_id']}")
    ...
    def update_buttons(self):
        games = self.get_games()
        total_pages = max(len(games), 1)
        for i, btn in enumerate(self.children[:4]):
            btn.disabled = (i < 2 and self.page <= 0) or (i >= 2 and self.page >= total_pages - 1)
    
    @ui.button(emoji="⏮️", style=discord.ButtonStyle.gray, custom_id="first_page")
    async def first_page(self, interaction: discord.Interaction, button: Button):
        self.page = 0
        self.update_buttons()
        await interaction.response.edit_message(embed=self.create_history_embed(), view=self)
    
    @ui.button(emoji="⬅️", style=discord.ButtonStyle.gray, custom_id="prev_page")
    async def prev_page(self, interaction: discord.Interaction, button: Button):
        self.page -= 1
        self.update_buttons()
        await interaction.response.edit_message(embed=self.create_history_embed(), view=self)
    
    @ui.button(emoji="➡️", style=discord.ButtonStyle.gray, custom_id="next_page")
    async def next_page(self, interaction: discord.Interaction, button: Button):
        self.page += 1
        self.update_buttons()
        await interaction.response.edit_message(embed=self.create_history_embed(), view=self)
    
    @ui.button(emoji="⏭️", style=discord.ButtonStyle.gray, custom_id="last_page")
    async def last_page(self, interaction: discord.Interaction, button: Button):
        self.page = len(self.get_games()) - 1
        self.update_buttons()
        await interaction.response.edit_message(embed=self.create_history_embed(), view=self)
    
    @ui.button(emoji="🌐", style=discord.ButtonStyle.gray, custom_id="switch_scope")
...
class WordleCog(commands.Cog):
    ...
    async def handle_end_game(self, interaction: discord.Interaction, won: bool):
        ...
        if settings["anonymous"]:
            ...
            embed.set_footer(text=f"Deine anonyme ID: {anon_id}")
        else:
    ...
    async def show_stats(self, interaction: discord.Interaction, user: discord.User):
        ...
        games = self.history.get_user_games(user.id, "global")
        embed = discord.Embed(
        ...
        if games:
            valid_games = [g for g in games if not g.get("anonymous", False)]
            recent = "\n".join(
                f"{datetime.fromisoformat(g['timestamp']).strftime('%d.%m %H:%M')}: "
                f"{g['word'].upper()} ({g['attempts']} Versuche) - "
                f"{'Anonym' if g.get('anonymous', False) else 'Öffentlich'}"
                for g in games[:10]
            )
            ...
            anon_id = f" ({settings['anon_id']})" if settings["anonymous"] else ""
            
            embed.description = (
                f"**Einstellungen:**\n"
                f"• Statistiken: {stats_status}\n"
                f"• Historie: {history_status}\n"
                f"• Anonymmodus: {anonymous_status}{anon_id}"
            )
            
            embed.add_field(name="Letzte Spiele", value=recent, inline=False)
            embed.add_field(name="Gewonnen", value=sum(g["won"] for g in valid_games), inline=True)
            embed.add_field(name="Verloren", value=len(valid_games)-sum(g["won"] for g in valid_games), inline=True)
            embed.add_field(name="Winrate", value=f"{sum(g['won'] for g in valid_games)/len(valid_games)*100:.1f}%" if valid_games else "0%", inline=True)
        else:
    ...
    async def show_history(self, interaction: discord.Interaction, user: discord.User):
        ...
        view = HistoryView(self, user.id, interaction.guild.id, "server")
...
class MainMenu(View):
    def __init__(self, cog):
        ...
        options = [
            discord.SelectOption(label="🏆 Leaderboard", value="leaderboard", emoji="🏆", description="Top-Spieler anzeigen"),
            discord.SelectOption(label="📊 Statistiken", value="stats", emoji="📊", description="Eigene Leistung sehen"),
            discord.SelectOption(label="📜 Historie", value="history", emoji="📜", description="Spielverläuche einsehen"),
            discord.SelectOption(label="⚙️ Einstellungen", value="settings", emoji="⚙️", description="Privatsphäre anpassen"),
            discord.SelectOption(label="❓ Hilfe", value="help", emoji="❓", description="Spielregeln lernen"),
            discord.SelectOption(label="🔍 Suche", value="search", emoji="🔍", description="Spieler finden")
        ]
        select = Select(
        ...
        select.callback = self.menu_select
        self.add_item(select)
    ...
    async def menu_select(self, interaction: discord.Interaction):
        ...
        handlers = {
            "leaderboard": lambda: self.cog.show_leaderboard(interaction),
            "stats": lambda: self.cog.show_stats.callback(self.cog, interaction),
            "history": lambda: self.cog.show_history.callback(self.cog, interaction),
            "settings": lambda: self.cog.user_settings.callback(self.cog, interaction),
            "help": lambda: self.cog.show_help(interaction),
            "search": lambda: self.cog.search_stats.callback(self.cog, interaction)
        }
...
class SettingsView(View):
    ...
    def add_buttons(self):
        ...
        for setting, label, row in buttons:
            btn = Button(
                label=f"{label} {'✅' if self.settings[setting] else '❌'}",
                style=discord.ButtonStyle.primary,
                row=row,
                emoji="⚙️",
